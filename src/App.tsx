...
export default function App() {
  ...
  const [cocinaAbierta, setCocinaAbierta] = useState<boolean>(true);
  ...
  useEffect(() => {
    const inicializar = async () => {
      try {
        ...
        // Cargar estado de cocina desde localStorage
        const estadoCocina = localStorage.getItem('cocinaAbierta');
        if (estadoCocina !== null) {
          setCocinaAbierta(JSON.parse(estadoCocina));
        }
      } catch (error) {
  ...
  const toggleCocina = () => {
    const nuevoEstado = !cocinaAbierta;
    setCocinaAbierta(nuevoEstado);
    setEstadoCocina(nuevoEstado);
  };
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 lg:gap-8 py-4 lg:py-6 border-b border-gray-200">
            ...
            <div className="flex items-center justify-between lg:justify-end gap-4 lg:gap-6">
              ...
              {/* Toggle de cocina abierta/cerrada */}
              <button
                onClick={toggleCocina}
                className={`px-4 lg:px-5 py-2.5 lg:py-3 rounded-xl flex items-center gap-2 transition-all ${
                  cocinaAbierta
              ...
              >
                ...
                <span className="text-sm lg:text-base">{cocinaAbierta ? 'Abierto' : 'Cerrado'}</span>
              </button>
            </div>
          </div>
